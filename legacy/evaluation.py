...
class ModelEvaluator:
    ...
    def evaluate_model(self, model_name: str, y_true: np.ndarray, 
                       ...
                       latency: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
        """
    ...
    def plot_latency_comparison(self, save: bool = True) -> plt.Figure:
        ...
        for bar, val in zip(bars, df['Mean']):
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + df['Std'].max()*0.1,
    ...
    def generate_report(self, save: bool = True) -> str:
        ...
        report_lines = [
            "=" * 60,
            "MISINFORMATION DETECTION MODEL EVALUATION REPORT",
            "=" * 60,
            ""
        ]
...
def evaluate_all_models(test_df: pd.DataFrame,
                        models: Dict[str, Any],
                        text_column: str = 'combined_text',
                        label_column: str = 'label') -> ModelEvaluator:
    """
