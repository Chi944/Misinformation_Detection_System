...
class ModelExplainer:
    ...
    def explain_with_lime(self, text: str, predict_fn, 
                          num_features: int = 10,
    ...
    def visualize_lime_explanation(self, explanation_result: Dict[str, Any],
    ...
    def explain_attention(self, bert_model, text: str,
    ...
    def visualize_attention(self, attention_result: Dict[str, Any],
                           save_path: Optional[Path] = None) -> plt.Figure:
        """
    ...
    def get_feature_importance_tfidf(self, model, top_n: int = 20) -> Dict[str, List[Tuple[str, float]]]:
        """
        ...
        return {
            'misinformation_indicators': list(zip(
                fi['misinformation_features'].tolist(),
                fi['misinformation_scores'].tolist()
            )),
            'credible_indicators': list(zip(
                fi['credible_features'].tolist(),
                fi['credible_scores'].tolist()
            ))
        }
    
    def visualize_feature_importance(self, feature_importance: Dict[str, Any],
                                     save_path: Optional[Path] = None) -> plt.Figure:
        """
    ...
    def generate_explanation_report(self, text: str, 
                                    traditional_model,
                                    bert_model=None,
        ...
        lime_result = self.explain_with_lime(
            text,
            lambda x: traditional_model.predict_proba(pd.Series(x))
        )
        ...
        if bert_model is not None and bert_model.is_trained:
            ...
            self.visualize_attention(
                attention_result,
                save_path=save_dir / 'attention_explanation.png'
            )
